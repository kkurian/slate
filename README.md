...
- **PR review state.** Give an issue a `pr:` — a number or a list — and the live server asks `gh` where each pull request's review stands, showing one aggregate glyph on the row (review pending, approved, changes requested, merged, or closed) and a per-reviewer breakdown in the issue page's Properties panel. It runs `gh` against the repo `slate.py` sits in; set `SLATE_REPO=owner/repo` to point elsewhere. The `gh` round-trips never sit on a page load: every render answers from last-known state while expired entries refresh on background threads, and the landed refresh live-reloads open pages. Like presence, it is display-only, never written to the markdown, absent from static builds, and fails soft — no `gh`, offline, or an unknown number simply drops that PR and the rest renders unchanged.
